...
class UbisModel{
    ...
    public function getAllUbis($fromcache = true){
        $param = [
        ];
        $method = str_replace(["\\","::"],"_",__METHOD__);
        $salt = "cache-data-".$method;
        ...
        if ($this->dataCache->hasItem($key) && $fromcache){
          ...
          return json_decode($data,true);
            ...
            $sql = "select a.*,b.name as PARENT_NAME,c.name as LEVEL_NAME from _ubis a left join _ubis b on b.code=a.parent left join _ubis_level c on c.code=a.level";
            // die($sql);
            $statement = $this->dbSys->createStatement($sql, $param);
            // Debug::dump($statement);die();
            $result    = $statement->execute();
    ...
    public function getUbisByCode($param = [],$fromcache = true){
        $method = str_replace(["\\","::"],"_",__METHOD__);
        $salt = "cache-data-".$method;
        $crypted1 = hash('sha1', $salt);
        $crypted2 = hash('sha1', json_encode($param));
        ...
        if ($this->dataCache->hasItem($key) && $fromcache){
          ...
          return json_decode($data,true);
            ...
            $sql = "select * from _ubis where code=:code";
            // die($sql);
            $statement = $this->dbSys->createStatement($sql, $param);
            // Debug::dump($statement);die();
            $result    = $statement->execute();
    ...
    public function selectAllUbis($fromcache = true){
        $param = [
        ];
        $method = str_replace(["\\","::"],"_",__METHOD__);
        $salt = "cache-data-".$method;
        ...
        if ($this->dataCache->hasItem($key) && $fromcache){
          ...
          return json_decode($data,true);
            ...
            $sql = "SELECT code as val,name as label from _ubis where STATUS=1";
            // die($sql);
            $statement = $this->dbSys->createStatement($sql, $param);
            // Debug::dump($statement);die();
            $result    = $statement->execute();
