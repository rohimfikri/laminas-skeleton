...
class RoleModel{
    ...
    public function getAllRole($fromcache = true){
        $param = [
        ];
        $method = str_replace(["\\","::"],"_",__METHOD__);
        $salt = "cache-data-".$method;
    ...
    public function getRoleByCode($param = [],$fromcache = true){
        $method = str_replace(["\\","::"],"_",__METHOD__);
        $salt = "cache-data-".$method;
        $crypted1 = hash('sha1', $salt);
        $crypted2 = hash('sha1', json_encode($param));
        ...
        if ($this->dataCache->hasItem($key) && $fromcache){
          ...
          return json_decode($data,true);
            ...
            $sql = "select * from _role where code=:code";
            // die($sql);
            $statement = $this->dbSys->createStatement($sql, $param);
            // Debug::dump($statement);die();
            $result    = $statement->execute();
    ...
    public function selectAllRole($fromcache = true){
        $param = [
        ];
        $method = str_replace(["\\","::"],"_",__METHOD__);
        $salt = "cache-data-".$method;
        ...
        if ($this->dataCache->hasItem($key) && $fromcache){
          ...
          return json_decode($data,true);
            ...
            $sql = "SELECT code as val,name as label from _role where STATUS=1";
            // die($sql);
            $statement = $this->dbSys->createStatement($sql, $param);
            // Debug::dump($statement);die();
            $result    = $statement->execute();
